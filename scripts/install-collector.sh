...
Optional environment variables:
  ...
  XRAY_MONITOR_PERIOD_DAYS, XRAY_MONITOR_RESET_ANCHOR,
  XRAY_WEBSITE_RETENTION_DAYS, CADDY_SITES_DIR
EOF
}
...
XRAY_WEBSITE_RETENTION_DAYS="${XRAY_WEBSITE_RETENTION_DAYS:-30}"
...
if [[ ! "${XRAY_WEBSITE_RETENTION_DAYS}" =~ ^[1-9][0-9]*$ ]]; then
  echo "XRAY_WEBSITE_RETENTION_DAYS must be a positive integer." >&2
  exit 1
fi
...
XRAY_WEBSITE_RETENTION_DAYS=${XRAY_WEBSITE_RETENTION_DAYS}
